
local ls = require("luasnip")
local s = ls.snippet
local t = ls.text_node
local i = ls.insert_node

return {
    -- numpy + pandas
    s("numpan", {
        t({
            "import numpy as np",
            "import pandas as pd",
            "", 
        }),
    }),

    -- matplotlib + japanize_matplotlib
    s("matja", {
        t({
            "import japanize_matplotlib  # noqa: F401",
            "import matplotlib.pyplot as plt",
            "",
        }),
    }),
    -- rich logger
    s("logger", {
        t({
          "import logging",
          "from rich.logging import RichHandler",
          "",
          "logger = logging.getLogger(__name__)",
          "logging.basicConfig(",
          "    level=logging.INFO,",
          '    format="%(message)s",',
          '    datefmt="[%X]",',
          "    handlers=[RichHandler(markup=True, rich_tracebacks=True)]",
          ")",
          "",
        }),
    }),
    -- main
    s("main", {
        t({ "def main():", "    " }),
        i(1),
        t({ "", "", "if __name__ == '__main__':", "    params = {", "    }", "    main(**params)" }),
    }),
    -- for
    s({ trig = "for", priority = 1001 }, {
        t({ "for "}),
        i(1),
        t({ " in range(" }),
        i(2),
        t({ ")"}),
    }),
    -- yes
    s("yes", {
        t('"Yes" if '),
        i(1),
        t(' else "No"'),
    }),
    -- MOD
    s("mod", {
        t('MOD = 10**9 + 7'),
    }),
    -- nmap
    s("ni", {
      t({
        "int(input())", 
        "", 
      }), 
    }),
    -- nm
    s("nm", {
      t({
        "map(int, input().split())", 
        "", 
      }), 
    }),
    -- nli
    s("nl", {
      t({
        "list(map(int, input().split()))", 
        "", 
      }), 
    }),
    -- inf
    s("inf", {
      t({
        'float("inf")'
      })
    }), 
    -- defaultdict
    s("imdd", {
      t({
        'from collections import defaultdict', 
        ''
      }), 
    }), 
    s("dd", {
      t({
        'defaultdict(', 
      }), 
      i(1), 
      t(')')
    }), 
    -- Counter
    s("imcter", {
      t({
        'from collections import Counter', 
        ''
      }), 
    }),
    s("cter", {
      t({
        'Counter(',
      }),
      i(1),
      t(')'),
    }),
    -- deque
    s("imdeq", {
      t({
        'from collections import deque'
      })
    }),
    -- stdin.readline (高速入力)
    s("sin", {
      t({
        'from sys import stdin',
        'input = stdin.readline',
        '',
      }),
    }),
    -- sys.setrecursionlimit
    s("rec", {
      t({
        'import sys',
        'sys.setrecursionlimit(',
      }),
      i(1, '10**6'),
      t(')'),
    }),
    -- heapq
    s("imhq", {
      t({
        'from heapq import heapify, heappush, heappop',
        ''
      }),
    }),
    -- SortedList
    s("imsl", {
      t({
        'from sortedcontainers import SortedList',
        ''
      }),
    }),
    s("sl", {
      t({
        'SortedList(',
      }),
      i(1),
      t(')'),
    }),
    -- 4方向 (上下左右)
    s("dxdy", {
      t('[(0, 1), (1, 0), (0, -1), (-1, 0)]'),
    }),
    -- Union-Find
    s("uf", {
      t({
        'class UnionFind:',
        '    def __init__(self, n: int) -> None:',
        '        self.parent = list(range(n))',
        '        self.size = [1] * n',
        '        self.group_count = n',
        '',
        '    def find(self, x: int) -> int:',
        '        while self.parent[x] != x:',
        '            self.parent[x] = self.parent[self.parent[x]]',
        '            x = self.parent[x]',
        '        return x',
        '',
        '    def union(self, x: int, y: int) -> bool:',
        '        rx, ry = self.find(x), self.find(y)',
        '        if rx == ry:',
        '            return False',
        '        if self.size[rx] < self.size[ry]:',
        '            rx, ry = ry, rx',
        '        self.parent[ry] = rx',
        '        self.size[rx] += self.size[ry]',
        '        self.group_count -= 1',
        '        return True',
        '',
        '    def same(self, x: int, y: int) -> bool:',
        '        return self.find(x) == self.find(y)',
        '',
        '    def group_size(self, x: int) -> int:',
        '        return self.size[self.find(x)]',
        '',
      }),
    }),
    -- BIT (Binary Indexed Tree / Fenwick Tree, 0-indexed)
    s("bit", {
      t({
        'class BIT:',
        '    """Binary Indexed Tree (Fenwick Tree, 0-indexed)．"""',
        '',
        '    def __init__(self, n: int) -> None:',
        '        self.n = n',
        '        self.tree = [0] * (n + 1)',
        '',
        '    def add(self, i: int, x: int) -> None:',
        '        """位置 i (0-indexed) に x を加算する．"""',
        '        i += 1',
        '        while i <= self.n:',
        '            self.tree[i] += x',
        '            i += i & -i',
        '',
        '    def sum(self, i: int) -> int:',
        '        """区間 [0, i) の総和を返す．"""',
        '        s = 0',
        '        while i > 0:',
        '            s += self.tree[i]',
        '            i -= i & -i',
        '        return s',
        '',
        '    def range_sum(self, left: int, right: int) -> int:',
        '        """区間 [left, right) の総和を返す．"""',
        '        return self.sum(right) - self.sum(left)',
        '',
      }),
    }),
    -- セグメント木 (一点更新・区間取得, 0-indexed)
    s("seg", {
      t({
        'class SegTree:',
        '    """セグメント木 (一点更新・区間取得, 0-indexed)．"""',
        '',
        '    def __init__(self, op, e, v) -> None:',
        '        self.op = op',
        '        self.e = e',
        '        v = [e] * v if isinstance(v, int) else list(v)',
        '        self.n = len(v)',
        '        self.size = 1',
        '        while self.size < self.n:',
        '            self.size <<= 1',
        '        self.data = [e] * (2 * self.size)',
        '        for i in range(self.n):',
        '            self.data[self.size + i] = v[i]',
        '        for i in range(self.size - 1, 0, -1):',
        '            self.data[i] = op(self.data[2 * i], self.data[2 * i + 1])',
        '',
        '    def update(self, i: int, x) -> None:',
        '        """位置 i の値を x に更新する．"""',
        '        i += self.size',
        '        self.data[i] = x',
        '        i >>= 1',
        '        while i:',
        '            self.data[i] = self.op(self.data[2 * i], self.data[2 * i + 1])',
        '            i >>= 1',
        '',
        '    def query(self, left: int, right: int):',
        '        """区間 [left, right) を op で畳み込んだ値を返す．"""',
        '        sl, sr = self.e, self.e',
        '        left += self.size',
        '        right += self.size',
        '        while left < right:',
        '            if left & 1:',
        '                sl = self.op(sl, self.data[left])',
        '                left += 1',
        '            if right & 1:',
        '                right -= 1',
        '                sr = self.op(self.data[right], sr)',
        '            left >>= 1',
        '            right >>= 1',
        '        return self.op(sl, sr)',
        '',
        '    def get(self, i: int):',
        '        """位置 i の値を返す．"""',
        '        return self.data[self.size + i]',
        '',
      }),
    }),
}
